...
const Page = () => {
  ...
  // Use of the useData hook to get the data from the context
  const { data } = useData();

  // Vérification si data et data.events existent, puis création d'une copie du tableau events
  // Check if data and data.events exist, then create a copy of the events array
  const last = data?.events
    // Crée une copie du tableau pour éviter de muter l'original
    // Create a copy of the array to avoid mutating the original
    ?.slice()

    // Trie les événements par date en ordre décroissant
    // Sort the events by date in descending order
    .sort((a, b) => new Date(b.date) - new Date(a.date))[0]; // Select the first element of the sorted array // Sélectionne le premier élément du tableau trié
  ...
  return (
    <>
      ...
      {/* Ajout data-testid=footer pour lest du footer */}
      {/* Add data-testid=footer for the rest of the footer */}
      <footer data-testid="footer" className="row">
        <div className="col presta">
          <h3>Notre derniére prestation</h3>

          {/* Vérifie si 'last' est défini */}
          {/* Check if 'last' is defined */}
          {!last ? (
            // Si 'last' n'est pas défini, affiche un message d'attente
            // If 'last' is not defined, display a waiting message
            "Waiting last event..."
          ) : (
            // Ajout de Modal. Si 'last' est défini, affiche une carte d'événement
            // Add Modal If 'last' is defined, display an event card
